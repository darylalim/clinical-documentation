...
- `clinical_ai/` — backend package. No Streamlit knowledge.
...
- `transcribe.py` — thin CLI shim over `clinical_ai.asr`.
- `tests/` — 21 unit tests + 1 gated integration test.
...
| Coverage | `uv run pytest --cov=clinical_ai --cov-report=term-missing` |
...
- `clinical_ai/` modules never import `streamlit`. `app.py` is the only Streamlit entry point.
- No PHI on disk. Audio bytes live in `st.session_state`; the only artifact is a user-initiated `.md` download.
- `clinical_ai/prompts.py` is the single source of truth for the SOAP system prompt. Iterate there, not inside `llm.py`.
